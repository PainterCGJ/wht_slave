...
ContinuityCollector::ContinuityCollector() : status_(CollectionStatus::IDLE), currentCycle_(0), lastActivePin_(-1)
{
...
bool ContinuityCollector::configure(const CollectorConfig &config)
{
    ...
    if (config.totalDetectionNum == 0 || config.totalDetectionNum > 65535)
    {
        return false;
    }
    ...
    // 优化内存分配策略 - 预分配所有内存
    {
        ...
        elog_i(TAG, "Memory allocated: %d rows x %d cols = %d elements (%d bytes)", config_.totalDetectionNum,
               config_.num, totalElements, totalBytes);
    }
