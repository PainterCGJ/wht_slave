...
void ltlpResetSendCtrl(LtlpSendCrtl *pCtrl)
{
    memset(pCtrl, 0, sizeof(LtlpSendCrtl));
}
void ltlpSenderHandleFrame(LtlpFrame *pFrame)
{
}
void ltlpStartToSend(uint8_t msgType, LtlpID_t destID, LtlpNeedAckTypeDef needAck, const uint8_t *payload,
                     uint32_t payloadLen)
{
    ...
    ltlpGetReadyToSend(&g_sendCtrl[LTLP_APP_FRAME], LTLP_APP_FRAME, msgType, destID, needAck, payload, payloadLen);
...
void ltlpSenderHandleSysFrame(LtlpFrame *pFrame)
{
    ...
    case SENDER_HANDSHKE: {
        ...
        else if (pFrame->header.info.msgType == LTLP_SYS_FRAME_NACK)
        {
            ...
            {
                g_callbacks[LTLP_CALLBACK_ON_HANDSHAKE_ERROR](pFrame, g_callbackParm[LTLP_CALLBACK_ON_HANDSHAKE_ERROR]);
            }
            ltlpSenderReset();
        }
        break;
    ...
    case SNEDER_WAIT_ACK: {
        if (pFrame->header.info.msgType == LTLP_SYS_FRAME_ACK)
        {
            ...
            if (g_sendCtrl[LTLP_APP_FRAME].sendFrameCnt == g_sendCtrl[LTLP_APP_FRAME].frame.header.info.totalFrames)
            {
                // 发送完成
...
SenderSateDef ltlpSenderRun()
{
    ...
    case SENDER_HANDSHKE: {
        ...
        if (ltlpNowTime() - g_lastSendTime >= LTLP_TIMEOUT_MS)
        {
            ...
            if (g_callbacks[LTLP_CALLBACK_ON_HANDSHAKE_ERROR] != NULL)
            {
                g_callbacks[LTLP_CALLBACK_ON_HANDSHAKE_ERROR](NULL, g_callbackParm[LTLP_CALLBACK_ON_HANDSHAKE_ERROR]);
            }
            ltlpSenderReset();
        }
        break;
    ...
    case SENDER_SENDING: {
        ...
        g_lastSendTime = ltlpNowTime();
        if (g_sendCtrl[LTLP_APP_FRAME].frame.header.info.needAck == LTLP_NEED_ACK_YES)
        {
        ...
        else
        {
            ...
            if (g_sendCtrl[LTLP_APP_FRAME].sendFrameCnt == g_sendCtrl[LTLP_APP_FRAME].frame.header.info.totalFrames)
            {
                ...
                {
                    g_callbacks[LTLP_CALLBACK_ON_SEND_ALL_DATA_DONE](
                        &g_sendCtrl[LTLP_APP_FRAME].frame, g_callbackParm[LTLP_CALLBACK_ON_SEND_ALL_DATA_DONE]);
                }
                ltlpSenderReset();
            }
        }
