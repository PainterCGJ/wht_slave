...
void ltlpKernelHandleFrame(LtlpFrame *pFrame)
{
    ...
    {
    case KERNEL_IDLE: {
        if (pFrame->header.info.frameType == LTLP_SYS_FRAME && pFrame->header.info.msgType == LTLP_SYS_FRAME_HANDSHAKE)
        {
            // 收到握手请求，回复ACK
    ...
    for (uint32_t i = 0; i < len; i++)
    {
        ...
        case KERNEL_PARSE_HEADER: {
            ...
            else
            {
                ...
                {
                    if (g_parserCtrl.frame.header.headerCRC == ltlpCrc((uint8_t *)g_parserCtrl.frame.header.headerBytes,
                                                                       sizeof(g_parserCtrl.frame.header.headerBytes)))
                    {
                            ...
                            g_parserCtrl.frame.header.info.destID != LTLP_BROADCAST_ID)
                        {
                            // 目标ID不匹配，丢弃该帧
                            ltlpResetParser();
                            ltlpLogger("ltlp kernel parse frame destID(%08X), localID(%08X), mismatch, drop frame",
                                       g_parserCtrl.frame.header.info.destID, g_setting.localID);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        ltlpLogger("ltlp frame check crc %x", ltlpCrc((uint8_t *)g_parserCtrl.frame.header.headerBytes,
                                                                      sizeof(g_parserCtrl.frame.header.headerBytes)));
            ...
            else if (g_parserCtrl.recvPayloadLen < g_parserCtrl.frame.header.info.payloadLen + 2)
            {
                ...
                if (crcIndex == 1)
                {
                    ...
                    else
                    {
                        ...
                        ltlpLogger(
                            "ltlp kernel parse frame check crc %x",
                            ltlpCrc(g_parserCtrl.frame.pFramePayload, g_parserCtrl.frame.header.info.payloadLen));
